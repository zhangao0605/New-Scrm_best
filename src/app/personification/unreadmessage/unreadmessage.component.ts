...
@Component({
  selector: 'app-unreadmessage',
  templateUrl: './unreadmessage.component.html',
  styleUrls: ['./unreadmessage.component.css']
})
export class UnreadmessageComponent implements OnInit {
  ...
  rightData=[];
  wechatid='';
  immediately:string=GlobalUrl.prioductUrl +'personification/viewUnreadMsg';
	  	 	      ...
	  	 	      var shejiaoids='';
	  	 	       for(var i=0;i<this.rightData.length;i++){
				       ...
				       shejiaoids=shejiaoids.substring(0,shejiaoids.length-1);
				        ...
				        this.unreadshejiao=str;
					  	  this.unreadshe=false;
					  	  this.ismask=false;
					  	  this.rightData=[];
					  	  this.sjinputmodel='';
					  	  for(var i=0;i<this.shejarr.length;i++){
            ...
            formdata1.append('accountId',localStorage.getItem('id'));
	         //任务名称
    	      if(this.unreadtext==''){
    	      	...
    	      	formdata1.append('taskName',this.unreadtext);
    	      }
    	      //社交号
    	      if(this.wechatid==''){
					  	 alert("请选择社交号");
