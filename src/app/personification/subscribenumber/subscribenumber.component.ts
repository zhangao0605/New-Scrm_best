...
@Component({
  selector: 'app-subscribenumber',
  templateUrl: './subscribenumber.component.html',
  styleUrls: ['./subscribenumber.component.css']
})
export class SubscribenumberComponent implements OnInit {
	ismask=false;
	subscrtext="";
	subscrshejiao="";
  subscrshe=false;
  ...
  rightData=[];
  wechatid='';
  immediately:string=GlobalUrl.prioductUrl +'personification/viewSubscriptionNumber';
	  	 	      ...
	  	 	      var shejiaoids='';
	  	 	       for(var i=0;i<this.rightData.length;i++){
				       ...
				       shejiaoids=shejiaoids.substring(0,shejiaoids.length-1);
				        ...
				        this.subscrshejiao=str;
					  	  this.subscrshe=false;
					  	  this.ismask=false;
					  	  this.rightData=[];
					  	  	this.sjinputmodel='';
					  	  for(var i=0;i<this.shejarr.length;i++){
  ...
  closesubscrshe(){
   ...
   subscrsure(){
    	      ...
    	      if(this.subscrtext==''){
    	      	...
    	      	formdata1.append('taskName',this.subscrtext);
    	      }
    	      //社交号
    	      if(this.wechatid==''){
					  	 alert("请选择社交号");
