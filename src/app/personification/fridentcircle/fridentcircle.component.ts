...
@Component({
  selector: 'app-fridentcircle',
  templateUrl: './fridentcircle.component.html',
  styleUrls: ['./fridentcircle.component.css']
})
export class FridentcircleComponent implements OnInit {
	  ismask=false;
    pengshe=false;
    item:number = 0;
    fridenttext="";
    ...
    firidentlistid='';
    immediately:string=GlobalUrl.prioductUrl +'personification/checkFriendCircle';
   ...
   fridentsure(){    
           ...
           //任务名称
    	      if(this.fridenttext==''){
    	      	...
    	      	formdata1.append('taskName',this.fridenttext);
    	      }
    	      //社交号
    	      if(this.wechatid==''){
					  	 alert("请选择社交号");
					  ...
					  //朋友圈
					  if(this.item==0){
					  	  formdata1.append('moment','0');
					  	  formdata1.append('wechat',this.wechatid);
					  }else{
					  	  formdata1.append('moment','1');
					  	if(this.firidentlistid==''){
					  		alert("请选择好友");
					  		return false;
					  	}else{
					  		formdata1.append('wechat',this.firidentlistid);
					  	}
					  }
            //查看数量
            ...
            if(this.seetime==''){
            	...
            	formdata1.append('time',this.seetime);
            }
    ...
    //确认显示
		surefrident(){
			if(this.sendcirRightData.length==0){
				...
				this.ishowlaqun=false;
				this.ismask=false;
				$(".specificfrident").html(str);
				this.firidentlistid=strid;
				console.log(this.firidentlistid)
			}
		}
   //删除
