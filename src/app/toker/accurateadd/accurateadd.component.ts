...
@Component({
  selector: 'app-accurateadd',
  templateUrl: './accurateadd.component.html',
  styleUrls: ['./accurateadd.component.css']
})
export class AccurateaddComponent implements OnInit {
	 public  emjoyAllGet = [
  ...
  zhouqiarr=[{zhouqiweek:"",time3:new Date(0, 0, 0, 0, 0, 0),time4:new Date(0, 0, 0, 0, 0, 0)}];
   ...
   //精准加人大接扣
   AccurateandUrl:string=GlobalUrl.prioductUrl +'extension/preciseAddPerson';
	  ...
	  shejiaolist(){
	         ...
	         this.http.post(this.shejiaoUrl,formdata1).subscribe(response=>{
	      	 if(response['code']==200){
	      	 	 this.totalone=response['data'].socialaccountCount;
   ...
   //话术列表
    showhuashu(){
         ...
         this.http.post(this.showHuaShu,formdata1).subscribe(response=>{
      	 if(response['code']==200){
      	 	  this.total=response['data'].scriptCount;
            ...
            // 目标微信/qq/手机号/判断
					   if(this.weiqpnum==0){
					   	  if(this.gaoxuquejia==''){
					   	  	...
					   	  	formdata1.append('wechat',this.gaoxuquejia);
					   	  }
				    ...
				    //话术判断
					   if(this.huashunum==0){
					     	var a=$("#huashucontent").html();
					   	  if(a==''){
					   	  	 alert("请选择话术标签");
					   	  	 return false;
					   	  }else{
					   	  	 formdata1.append('script',a);
					   	  }
