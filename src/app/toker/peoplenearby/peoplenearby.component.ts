...
@Component({
  selector: 'app-peoplenearby',
  templateUrl: './peoplenearby.component.html',
  styleUrls: ['./peoplenearby.component.css']
})
export class PeoplenearbyComponent implements OnInit {
    ...
    //选择社交号
	 ...
	 searchURL:string=GlobalUrl.prioductUrl +'socialaccount/getAllSocialaccountByNameTo';
	  ...
	  shejiao='';
	  taskmodel='';
	  AccurateandUrl:string=GlobalUrl.prioductUrl +'extension/peopleNearby';
     ...
     //立即执行
	  open(){
		   ...
		   if(this.taskmodel==""){
            ...
            //判断性别
            if(this.sexnum==0){
            	formdata1.append('sex','2');
            }else if(this.sexnum==1){
            	formdata1.append('sex','0');
            }else{
            	formdata1.append('sex','1');
            }
            
            //判断距离
            if(this.distancenum==1000){
            	formdata1.append('range','1000');
            }else if(this.distancenum==2000){
            	formdata1.append('range','2000');
            }else{
            	formdata1.append('range','5000');
            }
...
// 删除定时
 ...
 delectzhouqi(index){
   ...
   DistanceClick(value){
   	  this.distancenum = value;
	    console.log(this.distancenum);
   }
   //关闭社交号
   noispeoplenearby(){
   	  this.rightData=[];
