...
@Component({
  selector: 'app-forwardcircle',
  templateUrl: './forwardcircle.component.html',
  styleUrls: ['./forwardcircle.component.css']
})
export class ForwardcircleComponent implements OnInit {
	  radioValue="";
	  ismask=false;
    forwardshe=false;
    fridendshe=false;
   ...
   nthcircle='';
   //大接口
   lastbigUrl:string=GlobalUrl.prioductUrl +'operate/forwardingFriendsCircle';
		...
		surefrident(){
			if(this.sendcirRightData.length==0){
				 alert("请选择朋友圈所属社交好")
				  ...
				  $(".showshejiao").html(str);
				  this.fridendshe=false;
				  this.ismask=false;
				  this.sendcirRightData=[];
				  this.fridendsheids=strid;
				  console.log(str);
				  console.log(strid);
			}
		}
    //立即执行
		open(){
             ...
             //判断社交号
					  ...
					  //朋友圈所属社交号
					  if(this.fridendsheids==''){
					  	 alert("请选择朋友圈所属社交号");
					  	 return false;
					  }else{
					  	formdata1.append('wechat',this.fridendsheids);
					  }
					  //关键字
					  if(this.item==0){
					  	 formdata1.append('isOrder','0');
					  	 if(this.thekeyword==''){
					  	 	  alert("请输入关键字");
					  	    return false;
					  	 }else{
					  	  	formdata1.append('keyWord',this.thekeyword);
					  	 }
					  }else{
					  	 formdata1.append('isOrder','1');
					  	if(this.nthcircle==''){
					  		alert("请选择转发第几条朋友圈");
					  		return false;
					  	}else{
					  		formdata1.append('number',this.nthcircle);
					  	}
					  }
					    //超时时间
					   if(this.timeout==''){
